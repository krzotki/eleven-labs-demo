...
export const commonCustomFlow = async ({
  interaction,
  customMessage,
  options,
  voice,
  ...
  const chatCallback = async ({
    ...
    if (
      ...
      await reply(interaction, "🎤", false);
      return true;
    }

    // else just send the message
    await reply(interaction, "Join the voice channel first!", false);
    return false;
  };
  ...
  await increaseUsage({
    ...
    usageRecordId,
    voiceCharacters: isUsingDefaultElevenLabsVoices ? customMessage.length : 0,
    onlyVoice: true,
  });
